...
var g_IsTest = false;
var g_IsInteractive = true;
var g_arrayScene = [];
var g_arraySceneCamera = [];
...
function IsShowDefaultScene(){
	if(g_arrayScene.length > 0){
		return true;
	}
	return false;
}

function LoadSceneFrom360Photo(array360Photo){
...
function OnSetDefaultSceneView(){
	...
	rotation = x + " " + y + " " + z;
	g_arraySceneCamera[g_currentScene] = rotation;
	
	scenes[g_currentScene]['rotation'] = rotation;
}

function OnNavigationMenuCliked(){
	if(IsShowDefaultScene() == false && !g_IsTest || !g_IsInteractive)
		return;
...
function OnImageMenuCliked(){
	if(IsShowDefaultScene() == false && !g_IsTest || !g_IsInteractive)
		return;
	
	HideAllMenuDlg();
	$("#image-tab").css("opacity", "1");
	ShowAddPhotoContainer();
}

function OnVideoMenuCliked(){
	if(IsShowDefaultScene() == false && !g_IsTest || !g_IsInteractive)
		return;
	
	HideAllMenuDlg();
	$("#video-tab").css("opacity", "1");
	ShowAddVideoContainer();
}

function LightMenuBtns(){
...
function HideAllMenuDlg(){
	g_IsInteractive = false;
	$("#navigation-tab").css("opacity", "0.4");
	$("#image-tab").css("opacity", "0.4");
	$("#video-tab").css("opacity", "0.4");
	
	$("#navigate-container").css("display", "none");
